...
export default async function FestSection() {
  ...
  return (
    ...
    >
      <div className="relative z-10 flex flex-col items-start bg-black/5 backdrop-blur-2xl border border-black/10 rounded-3xl p-8 md:p-16 shadow-2xl">
        <ScrollAnimationWrapper direction="up" className="w-full">
          <div className="text-left mb-10">
            <h2 className="text-sm uppercase tracking-[0.3em] text-[#0F0B1E]/60 mb-4 font-sans font-bold">
              Flagship Annual Fest
            </h2>
            <h3 className="font-serif text-5xl md:text-7xl lg:text-8xl font-bold mb-6 text-[#0F0B1E]">
              {FEST_CONFIG.name}
            </h3>
            <p className="text-lg md:text-xl text-[#0F0B1E]/70 max-w-2xl font-light">
        ...
        <ScrollAnimationWrapper direction="up" delay={0.4} className="w-full">
          <div className="grid grid-cols-3 gap-4 md:gap-12 max-w-4xl mb-12">
            {FEST_CONFIG.stats.map((stat, index) => (
              <div key={index} className="text-left p-6 rounded-2xl bg-black/5 border border-black/5 hover:bg-black/10 transition-colors">
                <div className="text-3xl md:text-5xl font-bold text-[#0F0B1E] mb-2 font-serif">
                  {stat.value}
                </div>
                <div className="text-xs md:text-sm uppercase tracking-widest text-[#0F0B1E]/50">
                  {stat.label}
                </div>
              </div>
            ))}
